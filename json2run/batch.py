...
class Batch(Persistent):
    ...
    def initialize_experiments(self):
        ...
        # add repetitions
        rep = Discrete({"name": "repetition", "values": list(range(self["repetitions"]))})
        self.generator = And({})
    ...
    def completion(self, greedy=False):
        ...
        return (float(Experiment.get({"batch": self["_id"]}).count()) / (
        self.generator.count() * float(self["repetitions"]))) * 100.0
    ...
    def run(self, thread_n=cpu_count(), greedy=False):
        ...
        try:
            ...
            while self.generator.has_more() and not self.interrupted:
                ...
                generated_count += 1
                e = Experiment(self, self["executable"], next(self.generator))
            ...
            list([x.kill() for x in self.enqueued])
            list([x.kill() for x in self.running])
...
class Race(Batch):
    def __init__(self, new=True, **kwargs):
        ...
        if new:
            # check if all needed parameters are in place
            missing = False
            for needed in ["generator", "name", "executable", "repetitions", "initial_block", "instance_parameter",
                           "performance_parameter", "seed", "separator", "prefix"]:
                try:
                    self[needed] = kwargs[needed]
                except:
                    missing = True
    ...
    def initialize_experiments(self):
        ...
        while self.generator.has_more():
            n = ParameterList(next(self.generator))
        ...
        log.info("Race has %d configurations, %d instances (%d experiments for each configuration)." % (
        len(self.configurations), len(instances), len(instances) * int(self["repetitions"])))
        ...
        # create instance generator
        rep = Discrete({"name": "repetition", "values": list(range(int(self["repetitions"])))})
    ...
    def run(self, thread_n=cpu_count(), greedy=False, alpha=0.05):
        ...
        log.info("Running with %d parallel threads, alpha = %f and %s." % (
        thread_n, alpha, ("greedy" if greedy else "non greedy")))

        self.greedy = greedy
        self.alpha = alpha

        # initialize data structures for configurations
        self.racing = list(range(0, len(self.configurations)))
        ...
        try:
            ...
            while (len(self.racing) > 1 and self.inst_generator.has_more()):
                ...
                if not missing:
                    # increment iteration
                    current_inst = ParameterList(next(self.inst_generator))
                    started_iteration += 1
                ...
                missing = [x for x in [ParameterList(self.configurations[c]) for c in self.racing] if
                           x not in enqueued + on_db]
    ...
    def experiment_finished(self, experiment):
        ...
        if self.interrupted or experiment.interrupted:
            if not self.interrupted:
                log.error("Experiment %s from iteration %d exited with status %d!" % (
                experiment.parameters, experiment.iteration, experiment.status))
            self.finish_lock.release()
            return
    ...
    def prune_inferiors(self):
        ...
        for c in self.racing:
            ...
            parameters = dict(list(parameters.items()) + list({"batch": self.get_id()}.items()))
        ...
        for c in self.racing:
            log.info("%s: %s %s, (sum: %s)" % (
            c, self.experiments[c], conf_rank[self.racing.index(c)], sum_of_ranks[self.racing.index(c)]))
        ...
        if self.iterations_completed + 1 >= self["initial_block"]:
            ...
            if k > 2:
                ...
                # ties correction
                ties_correction = sum([sum([x ** 3 - x for x in t]) / (k - 1) for t in ties])
                ...
                if p_value <= self.alpha:
                    ...
                    t_student = tstudent.ppf(1 - self.alpha / 2, (n - 1) * (k - 1)) * sqrt(
                        2 * (n * a - sum([pow(rk, 2) for rk in sum_of_ranks])) / ((n - 1) * (k - 1)))
                    best_sum = min(sum_of_ranks)

                    log.info("t: %f" % (t_student))

                    for c in self.racing:
                        log.info("%d: (%f - %f = %f) vs. %f" % (c, sum_of_ranks[self.racing.index(c)], best_sum,
                                                                abs(sum_of_ranks[self.racing.index(c)] - best_sum),
                                                                t_student))

                    self.racing = [i for i in self.racing if
                                   abs(sum_of_ranks[self.racing.index(i)] - best_sum) <= t_student]
    ...
    def compute_ranks(self, experiments):
        ...
        inst_first = list(map(list, list(zip(*experiments))))
        inst_rank = [list(rankdata(l)) for l in inst_first]

        # results
        conf_rank = list(zip(*inst_rank))

        # compute ties
        ties = [[len([x for x in a if x == t]) for t in set(a)] for a in inst_rank]
